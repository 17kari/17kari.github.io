/*
arrastrar_gen.js
Arrastrar y soltar genitivos de latin, 2a decl. 
Drag and drop latin genetive words, 2. decl.
Author: Kari Lajunen 
*/
          ...
          function highlightWordEndings(list)
          {
            ...
            if (elements.length > 0)
            {
              ...
              if (elements[0].innerHTML.slice(-1) == "i" && elements.length == 4)
              { // -i
              ...
              else if ( elements[0].innerHTML.slice(-1) == "m" && elements.length == 2 )
              { // -orum
                for (var i = 0; i < elements.length; i++)
                {
                  var original = elements[i].innerHTML;
                  elements[i].innerHTML = original.slice(0, -4) + "<strong>-" + original.slice(-4) + "</strong>";
                }
              }
          ...
          function drop_1(ev)
          {
              ...
              if ( target_x ) 
              {
                  ...
                  var genetives_1_ul = document.getElementById('genetives_1');
                  var genetives_2_ul = document.getElementById('genetives_2');

                  console.log("data_text_content = " + data_text_content);

                  // Let's add the copy of the node (word) into the genetives_1 list 
                  // if it is empty 
                  if ( genetives_1_ul.getElementsByTagName('li').length == 0 ) 
                  {

                    // and if there is no similar words (endings) in other lists.
                    if ( ( genetives_2_ul.getElementsByTagName('li').length == 0 || 
                      genetives_2_ul.firstElementChild.innerHTML.slice(-1) != 
                      data_text_content.slice(-1) ) )
                    {
                      ...
                      // list called e.g. "genetives_1". 
                      var li_node = document.createElement("LI");  
                      var textnode = document.createTextNode(data_text_content);
                      li_node.appendChild(textnode);
                      // Let's add the created new li_node to the ul list which id is e.g. "genetives".
                      genetives_1_ul.appendChild(li_node);
                  ...
                  { // There are already items in the genetives_ul_1 list

                    var first_word = genetives_1_ul.firstElementChild.innerHTML;
                    ...
                    if ( first_word.slice(-1) == data_text_content.slice(-1) )
                    {
                      ...
                      console.log("genetives_1_ul.length = " + genetives_1_ul.length);
                  ...
                  highlightWordEndings(genetives_1_ul);
          ...
          function drop_2(ev)
          {
              ...
              if ( target_x ) 
              {
                  ...
                  // Let's add the copy of the node (word) into the genetives_2 list 
                  // if it is empty 
                  if ( genetives_2_ul.getElementsByTagName('li').length == 0 ) 
                  {
                    ...
                    // and if there is no similar words (endings) in other lists.
                    if ( ( genetives_1_ul.getElementsByTagName('li').length == 0 || 
                      genetives_1_ul.firstElementChild.innerHTML.slice(-1) != 
                      data_text_content.slice(-1) ) )
                    {
                      console.log("--- if block...")
                      ...
                      li_node.appendChild(textnode);
                      // Let's add the created new li_node to the ul list which id is e.g. "genetives".
                      genetives_2_ul.appendChild(li_node);
                  ...
                  { // There are already items in the genetives_ul_2 list

                    var first_word = genetives_2_ul.firstElementChild.innerHTML;
                    console.log("first_word: " + first_word);
                ...
                highlightWordEndings(genetives_2_ul);
