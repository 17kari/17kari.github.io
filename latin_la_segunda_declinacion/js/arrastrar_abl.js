/*
arrastrar_abl.js
Arrastrar y soltar ablativos de latin, 2a decl. 
Drag and drop latin ablative words, 2. decl.
Author: Kari Lajunen 
*/
          ...
          function highlightWordEndings(list)
          {
            ...
            if (elements.length > 0)
            {
              ...
              if (elements[0].innerHTML.slice(-1) == "o" && elements.length == 7)
              { // -o
              ...
              else if ( elements[0].innerHTML.slice(-1) == "s" && elements.length == 4 )
              { // -is
                for (var i = 0; i < elements.length; i++)
                {
                  var original = elements[i].innerHTML;
          ...
          function drop_1(ev)
          {
              ...
              if ( target_x ) 
              {
                  ...
                  var ablatives_1_ul = document.getElementById('ablatives_1');
                  var ablatives_2_ul = document.getElementById('ablatives_2');

                  console.log("data_text_content = " + data_text_content);

                  // Let's add the copy of the node (word) into the ablatives_1 list 
                  // if it is empty 
                  if ( ablatives_1_ul.getElementsByTagName('li').length == 0 ) 
                  {

                    // and if there is no similar words (endings) in other lists.
                    if ( ( ablatives_2_ul.getElementsByTagName('li').length == 0 || 
                      ablatives_2_ul.firstElementChild.innerHTML.slice(-1) != 
                      data_text_content.slice(-1) ) )
                    {
                      ...
                      // list called e.g. "ablatives_1". 
                      var li_node = document.createElement("LI");  
                      var textnode = document.createTextNode(data_text_content);
                      li_node.appendChild(textnode);
                      // Let's add the created new li_node to the ul list which id is e.g. "ablatives".
                      ablatives_1_ul.appendChild(li_node);
                  ...
                  { // There are already items in the ablatives_ul_1 list

                    var first_word = ablatives_1_ul.firstElementChild.innerHTML;
                    ...
                    if ( first_word.slice(-1) == data_text_content.slice(-1) )
                    {
                      ...
                      console.log("ablatives_1_ul.length = " + ablatives_1_ul.length);
                  ...
                  highlightWordEndings(ablatives_1_ul);
          ...
          function drop_2(ev)
          {
              ...
              if ( target_x ) 
              {
                  ...
                  // Let's add the copy of the node (word) into the ablatives_2 list 
                  // if it is empty 
                  if ( ablatives_2_ul.getElementsByTagName('li').length == 0 ) 
                  {
                    ...
                    // and if there is no similar words (endings) in other lists.
                    if ( ( ablatives_1_ul.getElementsByTagName('li').length == 0 || 
                      ablatives_1_ul.firstElementChild.innerHTML.slice(-1) != 
                      data_text_content.slice(-1) ) )
                    {
                      console.log("--- if block...")
                      ...
                      li_node.appendChild(textnode);
                      // Let's add the created new li_node to the ul list which id is e.g. "ablatives".
                      ablatives_2_ul.appendChild(li_node);
                  ...
                  { // There are already items in the ablatives_ul_2 list

                    var first_word = ablatives_2_ul.firstElementChild.innerHTML;
                    console.log("first_word: " + first_word);
                ...
                highlightWordEndings(ablatives_2_ul);
